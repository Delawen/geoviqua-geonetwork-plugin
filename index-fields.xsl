  ...
  <!-- If identification creation, publication and revision date
	...
	<xsl:template match="*" mode="metadata">
		...
		<xsl:for-each select="gmd:identificationInfo//gvq:GVQ_DataIdentification|
			gmd:identificationInfo//*[contains(@gco:isoType, 'GVQ_DataIdentification')]|
			...
			<!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
            <xsl:variable name="email" select="/gvq:GVQ_Metadata/gmd:contact[1]/gmd:CI_ResponsibleParty[1]/gmd:contactInfo[1]/gmd:CI_Contact[1]/gmd:address[1]/gmd:CI_Address[1]/gmd:electronicMailAddress[1]/gco:CharacterString[1]"/>
		...
		<xsl:for-each select="gvq:dataQualityInfo/*/gmd:report/*/gmd:result">
			<xsl:if test="$inspire='true'">
				...
				* INSPIRE Specification on <Theme Name> – <version> for CRS and GRID
				...
				-->
				<xsl:if test="(
					contains(gmd19157:DQ_ConformanceResult/gmd19157:specification/gmd:CI_Citation/gmd:title/gco:CharacterString, '1089/2010') or
					contains(gmd19157:DQ_ConformanceResult/gmd19157:specification/gmd:CI_Citation/gmd:title/gco:CharacterString, 'INSPIRE Data Specification') or
					contains(gmd19157:DQ_ConformanceResult/gmd19157:specification/gmd:CI_Citation/gmd:title/gco:CharacterString, 'INSPIRE Specification'))">
			...
			<xsl:for-each select="//gmd19157:specification/*/gmd:title/gco:CharacterString">
				<Field name="specificationTitle" string="{string(.)}" store="true" index="true"/>
			</xsl:for-each>
			
			<xsl:for-each select="//gmd19157:specification/*/gmd:date/*/gmd:date">
				<Field name="specificationDate" string="{string(gco:Date[.!='']|gco:DateTime[.!=''])}" store="true" index="true"/>
			</xsl:for-each>
			
			<xsl:for-each select="//gmd19157:specification/*/gmd:date/*/gmd:dateType/gmd:CI_DateTypeCode/@codeListValue">
				<Field name="specificationDateType" string="{string(.)}" store="true" index="true"/>
			</xsl:for-each>
		</xsl:for-each>
		<xsl:for-each select="gvq:dataQualityInfo/*/gmd19157:lineage/*/gmd19157:statement/gco:CharacterString">
