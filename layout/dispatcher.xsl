...
<xsl:stylesheet version="2.0"
	...
	xmlns:gn-fn-iso19139.geoviqua="http://geonetwork-opensource.org/xsl/functions/profiles/iso19139.geoviqua"
	...
	<xsl:template name="get-iso19139.geoviqua-configuration">
		<xsl:copy-of select="document('config-editor.xml')" />
	</xsl:template>


	<xsl:template name="dispatch-iso19139.geoviqua">
		<xsl:param name="base" as="node()" />
		<xsl:param name="overrideLabel" as="xs:string" required="no"
			select="''" />

		<xsl:apply-templates mode="mode-iso19139.geoviqua"
			select="$base">
	...
	<!-- Evaluate an expression. This is schema dependant in order to properly 
		set namespaces required for evaluate. "The static context for the expression 
		includes all the in-scope namespaces, types, and functions from the calling 
		stylesheet or query" http://saxonica.com/documentation9.4-demo/html/extensions/functions/evaluate.html 
		A node returned by evaluate will lost its context (ancestors). -->
	<xsl:template name="evaluate-iso19139.geoviqua">
		<xsl:param name="base" as="node()" />
		<xsl:param name="in" />
	...
	<xsl:template name="evaluate-iso19139.geoviqua-boolean">
		<xsl:param name="base" as="node()" />
		<xsl:param name="in" />
